...
bool init(char *path){
    ...
    destroySqList(L);
    destroySortedSqList(Ls);
    destroyHash(H,Load::flag_resume);
    Load::flag_resume=false;
    ...
    while(fgets(line,LETTERS_PER_LINE-letters,pf)!=NULL){//读取一行（可能是空行、可能读到一半单词、可能提前结束）
        ...
        while(sscanf(pline,"%s",word)!=-1){//读取单词（可能有标点符号、可能是一半单词）
            ...
            if(word[0]=='\0')
                continue;
            clock_t start=clock();
            loadSqList(L,word,pages,lines);
            clock_t end=clock();
            Load::ct1+=(double)(end-start)/CLOCKS_PER_SEC*1000.0;

            start=clock();
            insertHash(H,word,pages,lines);
            end=clock();
...
bool saveData(char *path){
    //char path[100]="/Users/lrg/save.dat";
